...
const expectedResult5 = [
  ...
  function fantasyOrScienceFictionAuthors() {
    const genreArray = ['Fantasia', 'Ficção Científica'];

    return books.filter(book => genreArray.includes(book.genre)).map(book => book.author.name).sort();
  }
...
function authorWith3DotsOnName() {
  return books.find(book => book.author.name[7] === '.').name;
}
